...
def delta_dirac_parameters_generator(
        t2_myelin:tuple,
        t2_ie:tuple,
        t2_csf:tuple,
    ...
    t2_csf=np.random.uniform(t2_csf[0],t2_csf[1])
    t2s=torch.tensor([t2_myelin,t2_ie,t2_csf])
    t1s=torch.tensor([t1,t1,t1])
    ...
    #ie_weight=np.random.uniform(0,1-myelin_weigth)
    #csf_weight=1-myelin_weigth-ie_weight
    # generar el agua a lo mas en un porcentaje del 40%
    #csf_weight=np.random.uniform(0,0.4)
    #fijamos el peso del agua en 0.5
    csf_weight=0.05
    ie_weight=1-csf_weight-myelin_weigth
    weights=torch.tensor([myelin_weigth,ie_weight,csf_weight])
    return refoc_angle,t2s,weights # Regresamos los tensores de los parámetros de la señal de resonancia magnética

def Parallel_dirac_signal_parameters_generator(
    ...
    parameters=Parallel(n_jobs=-1)(delayed(delta_dirac_parameters_generator)(t2_myelin,t2_ie,t2_csf,angle_boundaries,t1,myelin_range_constrain) for i in range(N))
...
parser.add_argument('--N',type=int,default=10,help='Número de señales a ser generadas por lado')
...
parser.add_argument('--t2_csf',nargs=2,type=float,default=(500,1000),help='Rango de valores de T2 para el csf')
parser.add_argument('--angle_boundaries',nargs=2,type=float,default=(160,161),help='Rango de valores de los ángulos de refocamiento')
...
if __name__=="__main__":
    with torch.no_grad():
        ...
        refoc_angles,t2s,weights=Parallel_dirac_signal_parameters_generator(
        ...
        t2_my, t2_ie, t2_csf = t2s[:,0].numpy(), t2s[:,1].numpy(), t2s[:,2].numpy()
        t2_myelin=t2_my.reshape(arguments.N,arguments.N,1)
        t2_ie=t2_ie.reshape(arguments.N,arguments.N,1)
        t2_csf=t2_csf.reshape(arguments.N,arguments.N,1)
        ...
        t2_csf_name=os.path.join(arguments.prefix,arguments.prefix+"_t2csf.nii")
        nib.save(nib.Nifti1Image(t2_myelin,np.eye(4)),t2_myelin_name)
        nib.save(nib.Nifti1Image(t2_ie,np.eye(4)),t2_ie_name)
        nib.save(nib.Nifti1Image(t2_csf,np.eye(4)),t2_csf_name)
